...
# {START}
   ...
   before_filter :initialize_instance_variables, :except => [:customer_display,:add_item_ajax, :print_receipt, :print_confirmed, :log]
   ...
   before_filter :crumble, :except => [:customer_display,:print, :print_receipt, :print_confirmed, :log]
...
#     puts "!!! returned Item: #{@item.sku}"
    ...
    if @item.class == Item and @item.behavior == 'gift_card' and @item.sku == "G000000000000" then
      ...
      item_type = ItemType.find_by_behavior('gift_card')
      item_type_id = item_type.id if item_type
      @item = Item.create(:sku => "G#{timecode}", :vendor_id => $Vendor.id, :tax_profile_id => zero_tax_profile.id, :name => "Auto Giftcard #{timecode}", :must_change_price => true, :item_type_id => item_type_id)
